...
- Observation (Milestone 2 review): k6 needs about 2.5 MiB per VU in the memory profile. With 256 KiB prompts at 50 requests per second and 30 seconds of mock latency (1,512 requests in flight), k6's maximum sampled working set was 3.7 GiB, so that profile's Job gets 6 GiB. The mock replicas stayed at about 24 MiB each under the same load.

- Observation (Milestone 2 review): kubelet rotates container logs at 10 MiB and `kubectl logs` returns only the current file. Recorded requests per plan are capped at 20,000 (about 7 MiB of START and PROBE lines), and a run is rejected if its records do not cover every request k6 counted.

- Observation (P1, promoted): tenant identity comes from API key metadata, and one conditional policy enforces a separate token limit per tenant. With proto-a limited to 200 tokens per minute, eight 26-token requests succeeded (the limit applies after each response, so the ninth request was the first refused) and then every request got 429 with `x-ratelimit-limit: 200`, `x-ratelimit-remaining: 0`, and `x-ratelimit-reset`, while proto-b kept getting 200. A key without metadata makes `apiKey.tenant == "..."` false rather than an error (the request got 404 from routing). The frontend metrics attribute `tenant` with expression `apiKey.tenant` labels every proxy metric; unauthenticated requests get `tenant="unknown"`. The metric names are `agentgateway_requests_total` (labels include `status`, `reason` such as `RateLimit` or `APIKeyAuth`, `route`, `backend`, and `tenant`), `agentgateway_request_duration_seconds` (histogram), and `agentgateway_gen_ai_client_token_usage` (histogram; `_sum` by `gen_ai_token_type` input or output). Access logs carry `tenant=` too. The `tokens` field is an int32 with minimum 1; the API server accepted 2147483648 in a dry run, so values are kept at or below 1,000,000,000.

- Observation (P2, promoted): per-tenant upstream selection works in the shared cluster. An AgentgatewayBackend with `ai.provider.openai`, `host: mock.mock-upstream.svc.cluster.local`, and `port: 8080` sends plain HTTP to the mock with the backend's own key (`policies.auth.secretRef`), replacing the tenant's gateway key, which never reaches the mock. The upstream receives the original path `/mock/v1/chat/completions` (no rewrite; the mock serves it). A PreRouting policy whose `transformation.request.set` sets `x-tenant` to `apiKey.tenant` runs after authentication and overwrites a client-supplied value: proto-a sending `x-tenant: proto-b` still reached proto-a's key. Client headers `x-mock-latency-ms` and `x-probe-id` reach the mock, and `x-mock-key-owner` and `x-mock-probe-id` come back. A backend whose secretRef names a missing Secret reports `Accepted=False` with reason `TranslationError`, and its requests get HTTP 500 "service not found": it fails closed and never sends the request without credentials.

- Observation (P3, promoted): two complete agentgateway installations run side by side in the dedicated cluster with deploy/agentgateway/tenant-values.yaml plus generated per-tenant values (`gatewayClassName: agw-<ns>`, `controllerName: agentgateway.dev/<ns>`, `rbac.gatewayNamespaces: [<ns>]`, `discoveryNamespaceSelectors: [{matchLabels: {kubernetes.io/metadata.name: <ns>}}]`, `monitoring.proxy.gatewayClassNames: [agw-<ns>]`). Each controller accepted only its own GatewayClass and programmed only its own Gateway, each proxy's `XDS_ADDRESS` points at its own controller, and Prometheus scraped both controllers and both proxies. `kubectl auth can-i` for proto-a's controller: list secrets in proto-b yes, patch gateways in proto-b no, create deployments in proto-b no. Uninstalling proto-b's release while proto-a served traffic lost no request (123 of 123 answered). Deleting a Gateway removes its proxy Deployment. Helm uninstall removes the release's cluster roles but not the controller-created GatewayClass, which must be deleted explicitly.

- Observation (P4, promoted): k6 2.3.0 pushes native histograms when `K6_FEATURES=native-histograms` is set (the older `K6_PROMETHEUS_RW_TREND_AS_NATIVE_HISTOGRAM` still works but prints "Legacy env var detected"), and Prometheus 3.14 ingests them through remote write. Over the real one-minute windows of a stream whose mock latency stepped from 50 to 500 milliseconds, `histogram_quantile(0.95, sum(increase(k6_http_req_duration_seconds{stream="p4"}[50s])))` gave 0.056 and 0.543 seconds. k6 wraps console output in logfmt unless run with `--log-format raw`; with it, each probe record is a plain `PROBE {json}` line. The custom counter `mtag_verdicts` arrives as `k6_mtag_verdicts_total` with `tenant`, `stream`, and `verdict` labels. Always-true thresholds on `http_reqs{stream:...}`, `dropped_iterations{scenario:...}`, and `mtag_verdicts{stream:...,verdict:...}` make the end-of-run summary report each stream separately. Both leak detector controls fired on every request: a wrong key owner (21 of 21) and a wrong echoed probe ID (21 of 21).

- Observation (P5, promoted): `proxy_config` returns the proxy's `/config_dump` through the Kind node. Each tenant's effective limit is at `.policies[] | select(.name.name == "tenant-limits") | .policy.traffic.localRateLimit[] | {condition, maxTokens: .pol[0].maxTokens, fillInterval: .pol[0].fillInterval}` (for example `apiKey.tenant == "proto-a"`, 200, `1m0s`). API key entries appear as `<redacted>`, and neither gateway keys nor provider keys appear anywhere in the dump.

- Observation (P6, promoted): `kill -KILL <pid>` inside the Kind node, with the process ID from `crictl inspect`, restarts the proxy container in place: the pod UID was unchanged, restartCount went from 0 to 1, lastState.terminated showed exit code 137 with finishedAt at the kill, and the proxy was ready again about 2 seconds later.
...
- Observation (final campaign): other software on the host saturated the machine during the first campaign attempt and put the 1-minute load average at 8 to 11 on 10 CPUs. The latency and flood calibrations then failed in the shared cluster (40 and 6,495 dropped iterations; p99 411 and 160 ms), while at 17:05 the same latency calibration had passed with a p99 of 106 ms.
  Evidence: `uptime` printed "load averages: 9.08 10.31 11.33", and `ps` showed the busiest processes outside Docker Desktop's virtual machine belonged to other software. The calibration gate rejected the runs, as designed.
...
- Decision (final campaign): every run also records the host's 1-minute load average, as evidence rather than a gate, and the campaign waits before each step until the host has settled (a 1-minute load average below 6 on 10 CPUs for three checks 20 seconds apart, or at most 15 minutes, because two idle clusters alone keep it near 5). The first campaign attempt was stopped and discarded.
  Rationale: host load from other software cannot be controlled from this repository, and a gate would also reject the heavy runs whose own load raises it; calibration and per-run delivery checks already reject runs whose apparatus it degraded.
  Date/Author: 2026-09-24, Copilot.
...
- Decision (Milestone 2): the load runner stops k6 early through k6's REST API (`PATCH /v1/status` with `stopped: true`), reached from the Kind node at the pod's address on port 6565, because deleting the Job would also delete the logs that hold the summary and probe records.
  Rationale: k6's control API needs an HTTP client, which the Kind node has and the k6 image may not; this adds no host port-forward. The API is unauthenticated inside the cluster, in the same accepted-risk category as the mock's admin endpoint.
  Date/Author: 2026-09-24, Copilot.
...
- Decision: a tenant is a client identity with exactly one API key and one token-per-minute limit that applies to the whole tenant. There are no users inside a tenant, no per-user limits, and no request-count limits.
  Rationale: the user asked to keep the spike as simple as possible. A multi-tenant gateway may later let tenant administrators define policies for their own users; that is recorded only as a structural finding for later.
  Date/Author: 2026-09-24, user.
